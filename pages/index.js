...
export default function Home() {
	...
	return (
		<div>
			...
			{/* Header */}
			<header className="sticky top-0 z-50 border-b-2 border-gray-100">
				{/* TODO: Navigation */}
				<nav className="flex w-full p-5 justify-between">
					...
					<div className="flex space-x-4 items-center">
						...
						<button onClick={toggle}>
							{showSideBar ? (
								<MenuAlt1Icon className="h-10 w-10 p-2 transition duration-150 transform hover:scale-110 cursor-pointer" />
							) : (
			...
			<body>
				{/* 1. SideMenu Bar */}
				{/* TODO: display 고치기 */}
				<div style={{ display: showSideBar ? 'block' : 'block' }} className="transition duration-150 border-r-2 border-red-400">
					<div>
							...
							<div className="sidebar-menu-items flex flex-col justify-between">
								<div>
									{SideBarData.map((item, index) => {
										return (
											<div
												className="flex items-center group cursor-pointer hover:bg-gray-200 focus:border-transparent border-l-4 border-blue-300 active:border-green-300"
												onClick={() => router.push(item.path)}
											>
												<div key={index} className={`$item.cName link h-12 w-12 p-3`} role="button">
													{item.icon}
												</div>
												<div className={showSideBar ? '' : 'hidden'}>{item.title}</div>
											</div>
										)
									})}
								</div>
								...
								<div className="border-t-2 border-gray-200">
									...
									<div style={{ display: showSideBar ? 'block' : 'none' }}>
										<div className="flex items-center justify-between p-6">
											<a className="flex space-x-2 link">
									...
									{/* showSideBar false일 때 */}
									<div style={{ display: showSideBar ? 'none' : 'block' }} className="h-12 w-12 p-3  border-l-4 border-transparent link hover:opacity-50 transform active:scale-90">
										<MdIcons.MdSettings size={iconSize} />
									</div>
								</div>
							</div>
						</nav>
						...
						<div className="flex flex-col justify-center bg-white px-6 py-5 rounded-lg m-10">
							<div>
								...
								<img
									...
									src="https://as1.ftcdn.net/jpg/01/66/50/68/500_F_166506838_2fbBwcQIbnkvnYeQYTFbFZSFE8UTWy4O.jpg"
									alt="cake logo"
									width={64}
									height={64}
								/>
